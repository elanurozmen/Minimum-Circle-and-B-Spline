...
{
    FILE *file;
    file=fopen("C:\\Users\\acer\\Desktop\\koordinat.txt","r"); //Dosyayi acar
    int a[15][2],noktasayisi=0,i=0,j=0;
    char sayix[20],sayiy[20];
    if((file=fopen("C:\\Users\\acer\\Desktop\\koordinat.txt","r"))==NULL)
    {
    ...
    for(int l=1; l<=20; l++) //x ve y eksenlerine esit 21 pixel aralikla kisa cizgiler cizer
    {
    ...
    for(i=0; i<noktasayisi; i++)
    {
        ...
        for(j=0; j<noktasayisi; j++) //birbirine en uzak 2 noktayi bulan dongu
        {
            if(cap<sqrt(pow((a[i][0]-a[j][0]),2)+pow((a[i][1]-a[j][1]),2)))  //birbirine en uzak 2 noktayi bulur
            {
    ...
    else
    {
        for(i=0; i<noktasayisi; i++)
        {
            kontrol = sqrt(pow((((y2-y1)*(a[i][0]))+((x1-x2)*(a[i][1]))+(y1*x2-x1*y2)),2))/(sqrt(pow(y2-y1,2))+pow(x1-x2,2)); //en uzak 2 noktaya en uzak 3. noktayi bulur
            if(h<kontrol)
            {
                h=kontrol;
        ...
        merkezx = ((pow(y1,2)+pow(x1,2))*(y2-y3)+ (pow(y2,2)+pow(x2,2))*(y3-y1)+(pow(y3,2)+pow(x3,2))*(y1-y2))/D;
        merkezy = ((pow(y1,2)+pow(x1,2))*(x3-x2)+ (pow(y2,2)+pow(x2,2))*(x1-x3)+(pow(y3,2)+pow(x3,2))*(x2-x1))/D;
        r=sqrt(pow(x2-merkezx,2)+pow(y2-merkezy,2)); //Yaricap bulur
